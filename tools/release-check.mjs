...
import { releaseCheckSuites } from './release-checks/index.mjs';

function releaseCheckSuiteGateScriptsForPhase(phase) {
  return releaseCheckSuites
    .filter((suite) => (suite.gatePhase ?? 'pre-live-shadow-synthetic') === phase)
    .flatMap((suite) => suite.gateScripts ?? []);
}

const releaseCheckSuitePreLiveShadowGateScripts = releaseCheckSuiteGateScriptsForPhase('pre-live-shadow-synthetic');
const releaseCheckSuitePostRealRedactedGateScripts = releaseCheckSuiteGateScriptsForPhase('post-real-redacted');
const releaseCheckSuiteRequiredManifestPaths = releaseCheckSuites.flatMap((suite) => suite.requiredManifestPaths ?? []);
...
const releaseGateScripts = [
  ...
  ...releaseCheckSuitePreLiveShadowGateScripts,
  ...
  ...releaseCheckSuitePostRealRedactedGateScripts,
];
...
function assertReleaseSuitesForPhase(phase, context) {
  for (const suite of releaseCheckSuites) {
    if ((suite.assertReleasePhase ?? 'final') === phase) suite.assertRelease?.(context);
  }
}
...
for (const suite of releaseCheckSuites) {
  suite.assertManifestMetadata?.({ manifest, manifestReleaseTag, assertIncludes, assertNotIncludes });
}
...
for (const requiredManifestPath of [
  'tools/release-checks/index.mjs',
  ...releaseCheckSuiteRequiredManifestPaths,
]) {
...
assertReleaseSuitesForPhase('pre-live-shadow-synthetic', { repoRoot, packageRoot, manifestReleaseTag, readJson, assert, assertIncludes, assertNotIncludes });
...
assertReleaseSuitesForPhase('post-real-redacted', { repoRoot, packageRoot, manifestReleaseTag, readJson, assert, assertIncludes, assertNotIncludes });
...
assertReleaseSuitesForPhase('final', { repoRoot, packageRoot, manifestReleaseTag, readJson, assert, assertIncludes, assertNotIncludes });
