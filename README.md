# Synaptic Mesh v0.1.12

Status: manual dry-run command contracts / pre-live shadow readiness release candidate `v0.1.12`; **not runtime-ready**; **not production/canary/enforcement-ready**.
...
Current v0.1.12 status is narrower:
...
- adds manual dry-run command/result contracts for a future human-invoked local dry-run over already-redacted bundles; this is schema/design/test evidence only and does not add a CLI implementation;
...
```bash
npm --prefix implementation/synaptic-mesh-shadow-v0 run release:check -- --target v0.1.12
```
