...
This matrix summarizes what the current Synaptic Mesh local-shadow package covers before a v0.1.5 release decision. It is intentionally conservative: **covered** means a tracked deterministic gate exists; **partial** means useful evidence exists but important semantics are still outside the gate; **pending** means planned or desired but not implemented; **out of scope** means not a goal for this release line.
...
| Offline real-flow replay | Real-flow replay gate (`test:real-flow-replay`) | Partial | Covers naturalistic handoff-like replay fixtures with gold labels, audit logs, scorecard metrics, and hash binding to linked parser evidence. It is fixture-driven replay, not an automatic receiver decision, live traffic, a classifier, or a live shadow observer. |
