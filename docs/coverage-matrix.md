...
| Live-input/source boundary contracts | Boundary contract gate (`test:live-input-source-boundary-contracts`) | Covered | 2 positive controls and 7 expected rejects requiring already-redacted input, explicit source tuple, record-only output, complete forbidden-effect tuple, and no live observer/runtime path. Local committed fixtures only; no live traffic, runtime, adapters, memory writes, approval, blocking/allowing, or enforcement. |
