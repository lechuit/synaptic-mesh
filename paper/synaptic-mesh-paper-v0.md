...
Boundary: local research only; not runtime, not publication, not operational use
...
This paper introduces **Synaptic Mesh**, a local protocol proposal for multi-agent memory authority. Borrowing from provenance and authorization traditions [R03, R04, R07, R08], the core claim is that memory claims should carry compact authority/status/boundary receipts that survive transformations such as source result → summary → handoff → next-action → action proposal. Receipts bind source identity, freshness, scope, promotion boundary, forbidden effects, later restrictive events, tuple completeness, and the proposed action boundary. The receiving agent checks these receipts before treating a memory-derived summary as permission, readiness, or durable truth.
