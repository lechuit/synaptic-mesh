...
v0.1.0-rc1 is currently validated in an OpenClaw-origin local shadow workflow. The included implementation is a standalone reference package and fixture suite, not an OpenClaw runtime integration. The intended direction is portable: other agent frameworks could implement the same receiver-side receipt validation discipline through adapters, but those adapters are future work.
